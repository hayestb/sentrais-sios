...
const TRACKS = {
  COMMERCIAL:  { id: "COMMERCIAL",  label: "Commercial Delivery",   color: "#1D4ED8", bg: "#DBEAFE", text: "#1E3A8A" },
  NONPROFIT:   { id: "NONPROFIT",   label: "Nonprofit / BGI / ARI", color: "#047857", bg: "#D1FAE5", text: "#064E3B" },
  GTM:         { id: "GTM",         label: "Go-to-Market",          color: "#B45309", bg: "#FDE68A", text: "#78350F" },
  FINANCIAL:   { id: "FINANCIAL",   label: "Financial & Quarterly", color: "#6D28D9", bg: "#EDE9FE", text: "#4C1D95" },
  GOVERNANCE:  { id: "GOVERNANCE",  label: "Legal / Governance",    color: "#B91C1C", bg: "#FEE2E2", text: "#7F1D1D" },
  INFRA:       { id: "INFRA",       label: "Infrastructure & IT",   color: "#0E7490", bg: "#CFFAFE", text: "#164E63" },
  PEOPLE:      { id: "PEOPLE",      label: "People & Org",          color: "#9D174D", bg: "#FCE7F3", text: "#831843" },
};

const ROLES = {
  TYE:      { id: "TYE",      label: "Tye Hayes — CEO / Founder",          color: "#1E3A8A" },
  ERIN:     { id: "ERIN",     label: "Erin — CTO / Delivery Gate",          color: "#1D4ED8" },
  MIKALINA: { id: "MIKALINA", label: "Mikalina — Sports / Atlanta POC",     color: "#064E3B" },
  VAL:      { id: "VAL",      label: "Val — Channel / Partnerships",         color: "#7C2D12" },
  DARRYL:   { id: "DARRYL",   label: "Darryl — SEAR / FEMA / Gov",          color: "#4C1D95" },
  FREE:     { id: "FREE",     label: "Free — Live Events / Strategic",       color: "#155E75" },
  ...
  COUNSEL:  { id: "COUNSEL",  label: "Chanise Anderson / BGI Counsel",       color: "#4B5563" },
  BGI_DIRS: { id: "BGI_DIRS", label: "BGI Independent Directors",            color: "#047857" },
};
...
const WEEKS = [
  ...
  },
  {
    ...
    events: [
      ...
      { id:"w11-5", track:"COMMERCIAL", title:"Sentrais Federal — OCI analysis + first engagement scoped", type:"ACTION", urgency:"MEDIUM", roles:["DARRYL","TYE"], leadDays:0, deps:["w8-4"], sla:"Tenant B Federal_Practice/ OCI firebreak must be confirmed first", earlyWarn:"" },
...
const GOLDEN_ROUTES = {
  ...
  MIKALINA: {
    ...
    keyDependencies: ["Erin: platform delivery", "Val: channel partnerships", "BGI Directors: program governance"],
...
const URGENCY_STYLE = {
  CRITICAL: { bg: "#FEE2E2", text: "#991B1B", dot: "#DC2626", border: "#FECACA", label: "CRITICAL" },
  HIGH:     { bg: "#FEF3C7", text: "#92400E", dot: "#D97706", border: "#FDE68A", label: "HIGH" },
  MEDIUM:   { bg: "#FEF9C3", text: "#713F12", dot: "#CA8A04", border: "#FEF08A", label: "MEDIUM" },
  LOW:      { bg: "#DCFCE7", text: "#14532D", dot: "#16A34A", border: "#BBF7D0", label: "LOW" },
};
...
const PHASE_META = {
  1: { color: "#1E40AF", border: "#BFDBFE", bg: "#EFF6FF", light: "#DBEAFE", label: "Phase 1: Foundation",    weeks: "Wk 1–4" },
  2: { color: "#065F46", border: "#A7F3D0", bg: "#ECFDF5", light: "#D1FAE5", label: "Phase 2: Acceleration",  weeks: "Wk 5–8" },
  3: { color: "#7C2D12", border: "#FDBA74", bg: "#FFF7ED", light: "#FED7AA", label: "Phase 3: Momentum",      weeks: "Wk 9–12" },
};
...
export default function MasterCalendarLight() {
  ...
  const allCritical = allEvents.filter(e => e.urgency === "CRITICAL");
  ...
  const depsEvents = allEvents.filter(e => e.deps.length > 0).map(e => {
    const wk = WEEKS.find(w => w.events.some(ev => ev.id === e.id));
    return { ...e, wk: wk?.wk, dates: wk?.dates, phase: wk?.phase };
  });
  ...
  return (
    <div style={{ fontFamily: "'Georgia', serif", background: "#F8F9FA", minHeight: "100vh", color: "#111827" }}>
      ...
      {/* HEADER */}
      <div style={{ background: "white", borderBottom: "3px solid #1E3A8A", padding: "18px 28px", boxShadow: "0 1px 4px rgba(0,0,0,0.08)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "12px" }}>
          <div>
            <div style={{ fontSize: "10px", letterSpacing: "3px", color: "#6B7280", fontFamily: "Arial, sans-serif", marginBottom: "4px" }}>
              SENTRAIS · NOVATELabs · BGI — INTERNAL USE ONLY
            </div>
            <h1 style={{ margin: 0, fontSize: "24px", fontWeight: "700", color: "#1E3A8A", letterSpacing: "-0.3px" }}>
              Unified Master Calendar
            </h1>
            <div style={{ color: "#6B7280", fontSize: "12px", marginTop: "3px", fontFamily: "Arial, sans-serif" }}>
              12-Week Sequencing Engine · Jun – Aug 2026 · 7 Tracks · 9 Roles · Lead Times · SLAs · Early Warning
            </div>
          </div>
          <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", alignItems: "center" }}>
            {[["timeline","Timeline"],["critical","Critical Path"],["golden","Golden Route"],["dependencies","Dependencies"]].map(([v,l]) => (
              <button key={v} onClick={() => setView(v)} style={{
                background: view === v ? "#1E3A8A" : "white",
                border: `1.5px solid ${view === v ? "#1E3A8A" : "#D1D5DB"}`,
                color: view === v ? "white" : "#374151",
                borderRadius: "6px", padding: "6px 14px", fontSize: "12px",
                cursor: "pointer", fontFamily: "Arial, sans-serif", fontWeight: view === v ? "600" : "400"
              }}>{l}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ background: "#FEE2E2", border: "1px solid #FECACA", borderRadius: "6px", padding: "8px 14px", marginTop: "12px", display: "flex", gap: "6px", flexWrap: "wrap", alignItems: "center" }}>
          <span style={{ fontSize: "11px", fontWeight: "700", color: "#991B1B", fontFamily: "Arial, sans-serif", flexShrink: 0 }}>⚠ ACTIVE CRITICAL ITEMS:</span>
          {allCritical.slice(0, 4).map(e => (
            <span key={e.id} style={{ fontSize: "11px", color: "#B91C1C", fontFamily: "Arial, sans-serif", background: "white", border: "1px solid #FECACA", borderRadius: "4px", padding: "1px 8px" }}>
      ...
      <div style={{ padding: "20px 28px", maxWidth: "1200px", margin: "0 auto" }}>
        ...
        {view === "timeline" && (
          <div>
            ...
            <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "20px" }}>
              <button onClick={() => setSelectedTrack("ALL")} style={{ background: selectedTrack === "ALL" ? "#1E3A8A" : "white", border: "1.5px solid #D1D5DB", color: selectedTrack === "ALL" ? "white" : "#374151", borderRadius: "20px", padding: "4px 14px", fontSize: "11px", cursor: "pointer", fontFamily: "Arial, sans-serif" }}>All Tracks</button>
              {Object.values(TRACKS).map(t => (
                <button key={t.id} onClick={() => setSelectedTrack(t.id)} style={{
                  background: selectedTrack === t.id ? t.bg : "white",
                  border: `1.5px solid ${selectedTrack === t.id ? t.color : "#D1D5DB"}`,
                  color: selectedTrack === t.id ? t.text : "#374151",
                  borderRadius: "20px", padding: "4px 14px", fontSize: "11px", cursor: "pointer", fontFamily: "Arial, sans-serif",
                  fontWeight: selectedTrack === t.id ? "600" : "400"
                }}>{t.label}</button>
              ))}
            </div>
            ...
            {[1, 2, 3].map(phase => {
              ...
              return (
                <div key={phase} style={{ marginBottom: "28px" }}>
                  {/* Phase header */}
                  <div style={{ background: pm.bg, border: `1px solid ${pm.border}`, borderLeft: `4px solid ${pm.color}`, borderRadius: "6px", padding: "10px 16px", marginBottom: "10px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <span style={{ fontSize: "13px", fontWeight: "700", color: pm.color, fontFamily: "Arial, sans-serif" }}>{pm.label}</span>
                      <span style={{ fontSize: "11px", color: "#6B7280", fontFamily: "Arial, sans-serif", marginLeft: "10px" }}>{phaseWeeks[0]?.dates} – {phaseWeeks[phaseWeeks.length-1]?.dates}</span>
                    </div>
                    <span style={{ background: pm.color, color: "white", fontSize: "10px", padding: "2px 10px", borderRadius: "12px", fontFamily: "Arial, sans-serif" }}>{pm.weeks}</span>
                  </div>

                  <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                    {phaseWeeks.map(wk => {
                      const critInWk = wk.events.filter(e => e.urgency === "CRITICAL");
                      const highInWk = wk.events.filter(e => e.urgency === "HIGH");
                      return (
                        <div key={wk.wk} style={{ background: "white", border: `1px solid ${expandedWeek === wk.wk ? pm.color : "#E5E7EB"}`, borderRadius: "8px", overflow: "hidden", boxShadow: expandedWeek === wk.wk ? `0 0 0 2px ${pm.border}` : "0 1px 3px rgba(0,0,0,0.05)" }}>
                          {/* Week header */}
                          <div onClick={() => setExpandedWeek(expandedWeek === wk.wk ? null : wk.wk)}
                            style={{ padding: "11px 16px", cursor: "pointer", display: "flex", alignItems: "center", gap: "14px", flexWrap: "wrap", background: expandedWeek === wk.wk ? pm.bg : "white" }}>
                            <span style={{ background: pm.color, color: "white", fontSize: "11px", fontWeight: "700", padding: "2px 10px", borderRadius: "4px", fontFamily: "Arial, sans-serif", flexShrink: 0 }}>WK {wk.wk}</span>
                            <span style={{ fontSize: "12px", color: "#6B7280", fontFamily: "Arial, sans-serif", flexShrink: 0 }}>{wk.dates}</span>
                            <div style={{ flex: 1, display: "flex", gap: "6px", flexWrap: "wrap" }}>
                              {critInWk.map(e => <span key={e.id} style={{ background: "#FEE2E2", color: "#991B1B", fontSize: "10px", padding: "1px 8px", borderRadius: "4px", fontFamily: "Arial, sans-serif" }}>⏰ {e.title.slice(0,40)}{e.title.length>40?"…":""}</span>)}
                              {highInWk.slice(0, 2).map(e => <span key={e.id} style={{ background: "#FEF3C7", color: "#92400E", fontSize: "10px", padding: "1px 8px", borderRadius: "4px", fontFamily: "Arial, sans-serif" }}>{e.title.slice(0,35)}{e.title.length>35?"…":""}</span>)}
                            </div>
                            <div style={{ display: "flex", gap: "6px", alignItems: "center", flexShrink: 0 }}>
                              <span style={{ fontSize: "11px", color: "#9CA3AF", fontFamily: "Arial, sans-serif" }}>{wk.events.length} events</span>
                              <span style={{ color: "#9CA3AF", fontSize: "12px" }}>{expandedWeek === wk.wk ? "▲" : "▼"}</span>
                            </div>
                          </div>

                          {expandedWeek === wk.wk && (
                            <div style={{ borderTop: `1px solid ${pm.border}`, padding: "10px 14px", display: "flex", flexDirection: "column", gap: "5px", background: "#FAFAFA" }}>
                              {wk.events.map(ev => {
                                ...
                                return (
                                  <div key={ev.id} onClick={() => setExpandedEvent(isEx ? null : ev.id)}
                                    style={{ background: "white", border: `1px solid ${isEx ? t.color : "#E5E7EB"}`, borderLeft: `3px solid ${t.color}`, borderRadius: "6px", padding: "9px 12px", cursor: "pointer", transition: "border-color 0.1s" }}>
                                    <div style={{ display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap" }}>
                                      <span style={{ fontSize: "14px", flexShrink: 0 }}>{EVENT_TYPE_ICON[ev.type]}</span>
                                      <span style={{ background: t.bg, color: t.text, fontSize: "10px", padding: "1px 7px", borderRadius: "10px", fontFamily: "Arial, sans-serif", flexShrink: 0, fontWeight: "600" }}>{t.label}</span>
                                      <span style={{ flex: 1, fontSize: "13px", color: "#111827", fontWeight: "500" }}>{ev.title}</span>
                                      <span style={{ background: u.bg, color: u.text, fontSize: "10px", padding: "1px 8px", borderRadius: "10px", fontFamily: "Arial, sans-serif", border: `1px solid ${u.border}`, flexShrink: 0 }}>{u.label}</span>
                                      <span style={{ fontSize: "10px", color: "#9CA3AF", fontFamily: "Arial, sans-serif", flexShrink: 0 }}>{ev.roles.join(" · ")}</span>
                                    </div>
                                    {isEx && (
                                      <div style={{ marginTop: "10px", paddingTop: "10px", borderTop: "1px solid #F3F4F6", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
                                        {ev.sla && <div style={{ fontSize: "12px" }}><span style={{ color: "#6D28D9", fontFamily: "Arial, sans-serif", fontWeight: "600" }}>SLA: </span><span style={{ color: "#374151" }}>{ev.sla}</span></div>}
                                        {ev.leadDays > 0 && <div style={{ fontSize: "12px" }}><span style={{ color: "#0E7490", fontFamily: "Arial, sans-serif", fontWeight: "600" }}>Lead time: </span><span style={{ color: "#374151" }}>{ev.leadDays} days</span></div>}
                                        {ev.earlyWarn && <div style={{ fontSize: "12px", gridColumn: "1/-1", background: "#FFFBEB", border: "1px solid #FDE68A", borderRadius: "4px", padding: "6px 10px" }}><span style={{ color: "#B45309", fontFamily: "Arial, sans-serif", fontWeight: "600" }}>⚠ Early Warning: </span><span style={{ color: "#78350F" }}>{ev.earlyWarn}</span></div>}
                                        {ev.deps.length > 0 && <div style={{ fontSize: "12px" }}><span style={{ color: "#6B7280", fontFamily: "Arial, sans-serif", fontWeight: "600" }}>Dependencies: </span><span style={{ color: "#374151" }}>{ev.deps.join(", ")}</span></div>}
                                      </div>
                                    )}
                                  </div>
                                );
        ...
        {view === "critical" && (
          <div>
            <div style={{ background: "#FEE2E2", border: "1px solid #FECACA", borderRadius: "8px", padding: "12px 16px", marginBottom: "20px" }}>
              <div style={{ fontSize: "12px", fontWeight: "700", color: "#991B1B", fontFamily: "Arial, sans-serif", marginBottom: "4px" }}>CRITICAL PATH — Deadlines, Gates & Milestones That Block Downstream Work</div>
              <div style={{ fontSize: "12px", color: "#B91C1C", fontFamily: "Arial, sans-serif" }}>Items shown include all CRITICAL urgency events plus all Gates and Milestones regardless of urgency level.</div>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              {WEEKS.flatMap(w => w.events.filter(e => e.urgency === "CRITICAL" || e.type === "GATE" || e.type === "MILESTONE").map(e => ({ ...e, wk: w.wk, dates: w.dates, phase: w.phase }))).map(ev => {
                const u = URGENCY_STYLE[ev.urgency];
                const t = TRACKS[ev.track];
                const pm = PHASE_META[ev.phase];
                return (
                  <div key={ev.id} style={{ background: "white", border: `1px solid ${ev.urgency === "CRITICAL" ? "#FECACA" : "#E5E7EB"}`, borderLeft: `4px solid ${ev.urgency === "CRITICAL" ? "#DC2626" : t.color}`, borderRadius: "8px", padding: "12px 16px", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
                    <div style={{ display: "flex", gap: "12px", alignItems: "flex-start", flexWrap: "wrap" }}>
                      <div style={{ flexShrink: 0 }}>
                        <span style={{ background: pm.bg, color: pm.color, fontSize: "10px", fontWeight: "700", padding: "2px 8px", borderRadius: "4px", fontFamily: "Arial, sans-serif", display: "block", marginBottom: "3px" }}>WK {ev.wk} · {ev.dates}</span>
                        <span style={{ fontSize: "16px" }}>{EVENT_TYPE_ICON[ev.type]}</span>
                      </div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: "14px", color: "#111827", fontWeight: "600", marginBottom: "5px" }}>{ev.title}</div>
                        {ev.sla && <div style={{ fontSize: "12px", color: "#6B7280", marginBottom: "3px", fontFamily: "Arial, sans-serif" }}><strong style={{ color: "#374151" }}>SLA:</strong> {ev.sla}</div>}
                        {ev.earlyWarn && <div style={{ fontSize: "12px", background: "#FFFBEB", border: "1px solid #FDE68A", borderRadius: "4px", padding: "4px 8px", color: "#78350F", fontFamily: "Arial, sans-serif" }}>⚠ {ev.earlyWarn}</div>}
                      </div>
                      <div style={{ textAlign: "right", flexShrink: 0 }}>
                        <div style={{ background: u.bg, color: u.text, fontSize: "10px", padding: "2px 10px", borderRadius: "10px", fontFamily: "Arial, sans-serif", border: `1px solid ${u.border}`, marginBottom: "4px" }}>{u.label}</div>
                        <div style={{ fontSize: "10px", color: "#9CA3AF", fontFamily: "Arial, sans-serif" }}>{ev.roles.join(" · ")}</div>
                      </div>
                    </div>
                  </div>
                );
        ...
        {view === "golden" && (
          <div>
            ...
            <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "20px" }}>
              {Object.values(ROLES).map(r => (
                <button key={r.id} onClick={() => setSelectedRole(r.id)} style={{
                  background: selectedRole === r.id ? r.color : "white",
                  border: `1.5px solid ${selectedRole === r.id ? r.color : "#D1D5DB"}`,
                  color: selectedRole === r.id ? "white" : "#374151",
                  borderRadius: "6px", padding: "5px 12px", fontSize: "11px", cursor: "pointer",
                  fontFamily: "Arial, sans-serif", fontWeight: selectedRole === r.id ? "700" : "400"
                }}>{r.id}</button>
              ))}
            </div>

            {/* Role card */}
            <div style={{ background: "white", border: `2px solid ${ROLES[selectedRole].color}20`, borderTop: `4px solid ${ROLES[selectedRole].color}`, borderRadius: "8px", padding: "20px 24px", marginBottom: "20px", boxShadow: "0 2px 8px rgba(0,0,0,0.08)" }}>
              <div style={{ fontSize: "10px", color: ROLES[selectedRole].color, letterSpacing: "2px", marginBottom: "6px", fontFamily: "Arial, sans-serif", fontWeight: "700" }}>GOLDEN ROUTE</div>
              <div style={{ fontSize: "18px", fontWeight: "700", color: "#111827", marginBottom: "14px" }}>{ROLES[selectedRole].label}</div>

              {roleRoute && (
                <div>
                  <div style={{ background: "#F9FAFB", borderRadius: "6px", padding: "12px 16px", marginBottom: "14px" }}>
                    <div style={{ fontSize: "10px", color: "#6B7280", fontFamily: "Arial, sans-serif", fontWeight: "700", letterSpacing: "1px", marginBottom: "4px" }}>DAILY RHYTHM</div>
                    <div style={{ fontSize: "13px", color: "#374151", lineHeight: 1.7 }}>{roleRoute.dailyRhythm}</div>
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
                    <div>
                      <div style={{ fontSize: "10px", color: "#6B7280", fontFamily: "Arial, sans-serif", fontWeight: "700", letterSpacing: "1px", marginBottom: "6px" }}>THIS ROLE OWNS</div>
                      {roleRoute.weeklyOwns.map((o, i) => (
                        <div key={i} style={{ display: "flex", gap: "8px", padding: "4px 0", borderBottom: "1px solid #F3F4F6", alignItems: "flex-start" }}>
                          <span style={{ color: ROLES[selectedRole].color, fontSize: "12px", flexShrink: 0, marginTop: "1px" }}>→</span>
                          <span style={{ fontSize: "12px", color: "#374151", lineHeight: 1.5 }}>{o}</span>
                        </div>
                      ))}
                    </div>
                    <div>
                      <div style={{ fontSize: "10px", color: "#6B7280", fontFamily: "Arial, sans-serif", fontWeight: "700", letterSpacing: "1px", marginBottom: "6px" }}>KEY DEPENDENCIES</div>
                      {roleRoute.keyDependencies.map((d, i) => (
                        <div key={i} style={{ fontSize: "12px", color: "#374151", padding: "4px 0", borderBottom: "1px solid #F3F4F6" }}>
                          <span style={{ color: "#6D28D9" }}>⟶ </span>{d}
                        </div>
                      ))}
                      <div style={{ fontSize: "10px", color: "#B91C1C", fontFamily: "Arial, sans-serif", fontWeight: "700", letterSpacing: "1px", marginBottom: "6px", marginTop: "14px" }}>EARLY WARNINGS</div>
                      {roleRoute.earlyWarnings.map((w, i) => (
                        <div key={i} style={{ fontSize: "12px", background: "#FEE2E2", border: "1px solid #FECACA", borderRadius: "4px", padding: "5px 8px", color: "#7F1D1D", marginBottom: "4px", fontFamily: "Arial, sans-serif" }}>
                          ⚠ {w}
                        </div>
                      ))}
                    </div>
            ...
            {/* Role events */}
            <div style={{ fontSize: "11px", color: "#6B7280", letterSpacing: "1px", fontFamily: "Arial, sans-serif", fontWeight: "700", marginBottom: "10px" }}>CALENDAR EVENTS — WEEKS 1–12</div>
            <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
              {roleEvents.map(ev => {
                ...
                const pm = PHASE_META[ev.phase];
                return (
                  <div key={ev.id} style={{ background: "white", border: "1px solid #E5E7EB", borderLeft: `3px solid ${t.color}`, borderRadius: "6px", padding: "9px 14px", display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap" }}>
                    <span style={{ background: pm.bg, color: pm.color, fontSize: "10px", fontWeight: "700", padding: "1px 8px", borderRadius: "4px", fontFamily: "Arial, sans-serif", flexShrink: 0 }}>WK {ev.wk} · {ev.dates}</span>
                    <span style={{ fontSize: "13px", flexShrink: 0 }}>{EVENT_TYPE_ICON[ev.type]}</span>
                    <span style={{ background: t.bg, color: t.text, fontSize: "10px", padding: "1px 7px", borderRadius: "10px", fontFamily: "Arial, sans-serif", flexShrink: 0 }}>{t.label}</span>
                    <span style={{ flex: 1, fontSize: "13px", color: "#111827" }}>{ev.title}</span>
                    <span style={{ background: u.bg, color: u.text, fontSize: "10px", padding: "1px 8px", borderRadius: "10px", border: `1px solid ${u.border}`, fontFamily: "Arial, sans-serif", flexShrink: 0 }}>{u.label}</span>
                    {ev.leadDays > 0 && <span style={{ fontSize: "10px", color: "#0E7490", fontFamily: "Arial, sans-serif", flexShrink: 0, background: "#CFFAFE", padding: "1px 7px", borderRadius: "10px" }}>+{ev.leadDays}d lead</span>}
                  </div>
                );
              })}
            </div>
        ...
        {view === "dependencies" && (
          <div>
            <div style={{ background: "#EDE9FE", border: "1px solid #C4B5FD", borderRadius: "8px", padding: "12px 16px", marginBottom: "20px" }}>
              <div style={{ fontSize: "12px", fontWeight: "700", color: "#4C1D95", fontFamily: "Arial, sans-serif" }}>DEPENDENCY MAP — What Blocks What</div>
              <div style={{ fontSize: "12px", color: "#6D28D9", fontFamily: "Arial, sans-serif" }}>Every event shown has at least one blocker. Lead times show how far in advance the work must begin.</div>
            </div>
            {depsEvents.map(ev => {
              const t = TRACKS[ev.track];
              const pm = PHASE_META[ev.phase];
              const u = URGENCY_STYLE[ev.urgency];
              return (
                <div key={ev.id} style={{ background: "white", border: "1px solid #E5E7EB", borderRadius: "8px", padding: "12px 16px", marginBottom: "6px", boxShadow: "0 1px 2px rgba(0,0,0,0.04)" }}>
                  <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", marginBottom: "8px", alignItems: "center" }}>
                    <span style={{ background: pm.bg, color: pm.color, fontSize: "10px", fontWeight: "700", padding: "2px 8px", borderRadius: "4px", fontFamily: "Arial, sans-serif", flexShrink: 0 }}>WK {ev.wk} · {ev.dates}</span>
                    <span style={{ background: t.bg, color: t.text, fontSize: "10px", padding: "1px 7px", borderRadius: "10px", fontFamily: "Arial, sans-serif", flexShrink: 0 }}>{t.label}</span>
                    <span style={{ flex: 1, fontSize: "13px", color: "#111827", fontWeight: "600" }}>{ev.title}</span>
                    <span style={{ background: u.bg, color: u.text, fontSize: "10px", padding: "1px 8px", borderRadius: "10px", border: `1px solid ${u.border}`, fontFamily: "Arial, sans-serif", flexShrink: 0 }}>{u.label}</span>
                  </div>
                  <div style={{ paddingLeft: "12px", borderLeft: "2px solid #E5E7EB" }}>
                    <div style={{ fontSize: "11px", color: "#6D28D9", fontFamily: "Arial, sans-serif", fontWeight: "700", marginBottom: "5px" }}>REQUIRES COMPLETION OF:</div>
                    <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                      {ev.deps.map(d => {
                        const dep = allEvents.find(e => e.id === d);
                        return dep ? (
                          <span key={d} style={{ background: "#EDE9FE", color: "#4C1D95", fontSize: "11px", padding: "3px 10px", borderRadius: "4px", fontFamily: "Arial, sans-serif" }}>
                            {dep.title.length > 55 ? dep.title.slice(0, 52) + "…" : dep.title}
                          </span>
                        ) : null;
                      })}
                    </div>
                    {ev.leadDays > 0 && (
                      <div style={{ marginTop: "6px", fontSize: "11px", color: "#0E7490", fontFamily: "Arial, sans-serif" }}>
                        <strong>Lead time required:</strong> {ev.leadDays} days before event date
                      </div>
                    )}
                  </div>
                </div>
      ...
      <div style={{ borderTop: "1px solid #E5E7EB", padding: "12px 28px", background: "white", display: "flex", justifyContent: "space-between", fontSize: "11px", color: "#9CA3AF", flexWrap: "wrap", gap: "8px", fontFamily: "Arial, sans-serif" }}>
        <span>SENTRAIS-CAL-2026-v1.0 (Light) · 12 weeks · {allEvents.length} events · {allCritical.length} critical · 9 roles · backward-mapped</span>
        <span style={{ color: "#B91C1C", fontWeight: "600" }}>⚠ 83(b) DEADLINE: Jun 28 · NFL GO-LIVE: Jun 30 · BGI FORMATION ACTIVE</span>
      </div>
    </div>
  );
}
